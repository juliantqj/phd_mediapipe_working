...
This is mediapipe multipose with Holistic (fslower)
...
export_data = 0   # 2 to export the data, 0 to avoid
...
complexity = 1  # complexity from 0 to 2  (more time but better accuracy)
...
MASK = 1  # 1 for the mask, 0 for full video
...
out = cv2.VideoWriter((dirVID + file_name +'_Pose_output.mp4'), fourcc, fps, (int(cap.get(3)), int(cap.get(4))))
out_black = cv2.VideoWriter((dirVID + file_name +'_Pose_output_black.mp4'), fourcc, fps, (int(cap.get(3)), int(cap.get(4))))
...
with mp_holistic.Holistic(   
    ...
    min_tracking_confidence=beta, # Setting it to a higher value can increase robustness of the solution, at the expense of a higher latency
    # complexity from 0 to 2  (more time but better accuracy)
    model_complexity=complexity, # Landmark accuracy as well as inference latency generally go up with the model complexity
...
#    results = holistic.process(image)
  #   pose_coord = results.pose_landmarks.landmark
...
#        image, results.right_hand_landmarks, mp_holistic.HAND_CONNECTIONS)
 ...
 #       df.to_csv((dirDAT + file_name +'_Pose_out_dat.csv'), mode='a', sep=',', index=False, header=False)    
    ...
    # write the frame and export the mp4 file with a the original video as background
        out.write(image)
