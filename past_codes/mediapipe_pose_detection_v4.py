...
for video_file_path in video_files:
    ...
    # dirDATworld = 'videos/export_dat/' + video_file_name + '/' + 'world/'

    if not os.path.exists(dirDAT):
        os.makedirs(dirDAT)
    # if not os.path.exists(dirDATworld):
    #     os.makedirs(dirDATworld)
    ...
    with PoseLandmarker.create_from_options(options) as landmarker:
        ...
        while cap.isOpened():
            ...
            if MASK == 1 and left or right or top or bottom > 0:  # if MASK YES
                # recreate the timestamp in ms for each frame
                ...
                text_position_frame = (background_position_frame[0] + background_padding_x, background_position_frame[1]
                                       + text_height + background_padding_y)
                text_color = (0, 0, 255)  # Red (B,G,R) convention
                cv2.putText(image, frame_number_text, text_position_frame, text_font, text_scale,
            ...
            # Combine into one big temp df
            # temp_pose_df1 = pd.concat([temp_pose_df1, pivot_df1])
            # temp_pose_df2 = pd.concat([temp_pose_df2, pivot_df2])
            ...
            '''
            ^^^ change the above to export df to csv after exiting the loop ^^^
            '''
...
temp_pose_df1 = pd.DataFrame(temp_pose_df1_list)
temp_pose_df2 = pd.DataFrame(temp_pose_df2_list)
